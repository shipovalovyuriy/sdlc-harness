...
| Session checkpoint | Surface context/time/cycle degradation early without taking control away from the user. | WARN only; finish the safe atomic step and ask `continue_current` or `start_fresh`. Do not auto-interrupt or auto-start a chat. |
